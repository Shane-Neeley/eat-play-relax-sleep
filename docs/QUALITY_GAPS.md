...
| 2 | **Arrangement intelligence beyond entrances, balance, and pan** | `source-sketch` preserves performances and supports one-pass, call-response, and whole-phrase loop shapes, but it does not yet infer harmony, phrase boundaries, free-time form, or a musically responsive accompaniment from audio. | Add explicit, inspectable observation artifacts for pitch/harmony, phrase landmarks, and tempo ambiguity, then author an arrangement that uses them without automatic tuning, quantizing, or destructive edits. Compare it against the current control in both audition orders. |
...
- Run the complete media suite before a checkpoint. Its current 184-test
  baseline is about 4 minutes 26 seconds, plus five JavaScript tests.
