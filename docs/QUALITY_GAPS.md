...
| 2 | **Arrangement intelligence beyond entrances, balance, and pan** | `source-sketch --observation` now checksum-binds exact phrase/pitch/pulse evidence and uses one measured phrase as an unchanged excerpt. It still does not infer free-time form, choose harmony, or author accompaniment that follows the observed pitch/pulse possibilities. | With a real performance, compare an observation-bound phrase pass against the whole-source control in both audition orders. Then author one explicit harmonic or free-time response from retained alternatives without automatic tuning or quantizing. |
