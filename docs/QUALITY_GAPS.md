...
| 2 | **Arrangement intelligence beyond entrances, balance, and pan** | `source-sketch --observation` checksum-binds phrase/pitch/pulse evidence and uses one substantial measured phrase unchanged. A private user-owned A/B now holds seed, bed, entrance, gain, and pan constant between a whole-source segment and observed phrase; its counterbalanced listening decision remains honestly pending. It still does not infer free-time form, choose harmony, or author accompaniment from observed alternatives. | Listen to the prepared control and phrase pass in both orders and record which relationship serves the song. Then author one explicit harmonic or free-time response from retained alternatives without automatic tuning or quantizing. |
...
- Run the complete media suite before a checkpoint. Its current 189-test
  baseline is about 4 minutes 57 seconds, plus five JavaScript tests.
