# Ten Count Switch — production patterns (2026-08-28)

## Creative seed and orthogonality

The Daily context at [shaneneeley.com](https://shaneneeley.com/) suggested that motion can begin before explanation. The release abstracts that idea into a body-level step that gains meaning through repetition and one changed switch; no passage, name, private detail, or recognizable phrasing is used publicly.

This is rotation slot 3 (dance mechanics). It deliberately moves away from the recent 11/8 field-call/rattle releases, 9/8 string-box, 5/4 filament/comet, particle, and waterline lanes: 126 BPM, 10/8 grouped 3+3+2+2, a source-free instrumental, and a wide two-shoe footwork film.
...
EPRS request/research/check, creative-quality approval, render/analyze, mix review, 24-bit master approval, representative-frame review, full decode, picture review, YouTube assembly/approval, thumbnail/caption/chapter bundle review, release packaging, publication preparation, and frontier validation passed. A first browser file-chooser bridge returned `Not allowed`; the documented Computer Use native picker completed the upload and thumbnail selection without a duplicate upload. Future runs should preflight that bridge before expensive rendering.
