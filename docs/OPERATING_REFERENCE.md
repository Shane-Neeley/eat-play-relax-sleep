# Detailed workflow reference (preserved September 4, 2026)

This preserves the pre-refactor operating contract, including local work already in progress. Use it for command details and artifact contracts, not as a mandate to stop after one experiment or ask again for authorization already supplied. The current entry contract is ../AGENTS.md.
...
19. Keep `song-manifest.json` current. Successful and nonzero mutating EPRS
    command attempts are ledgered automatically; use `manifest record` for
    external software, human performance, considered/rejected methods, and the
    reason behind them. Use `manifest note` for prompts, ideas, thoughts, and
    other loose context that should remain open-ended. Treat `not-evidenced` as
    an audit gap or exploration lead, never proof that a method was absent.
...
scripts/eprs manifest build songs/<song-name> --probe-tools
scripts/eprs manifest record --song songs/<song-name> --method "Sonic Pi live-code" --status used --reason "Why this route served the song" --setting '{"seed":23}'
scripts/eprs manifest note --song songs/<song-name> --section "ideas for an opposite version" --text "One loose idea that should survive."
scripts/eprs manifest verify songs/<song-name>
